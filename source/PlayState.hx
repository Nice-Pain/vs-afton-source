...
class PlayState extends MusicBeatState
{
	...
	public var scanline:FlxSprite;
	public var arcadeoverlay:FlxSprite;
	public var blackStartBG:FlxSprite;
	...
	var bb:FlxSprite;
	var chica:FlxSprite;
	var nightNewGlitch:FlxSprite;
	var nightNewSprite:FlxSprite;
	var salvageBlack:FlxSprite;
	var canGlitchSalvage:Bool;
	var nightStart:Bool;
	var sixAM:FlxSprite;
	var glitchSixAM:FlxSprite;
	var glitch6amY:Int = 0;
	var flashSprite:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width * 3, FlxG.height * 3, 0xFFb30000);
	var gradientSalvage:BGSprite;
	var smokeNight:BGSprite;
	var lightNight:BGSprite;
	var springStatic:FlxSprite;
	var emitterSpring:FlxEmitter;
	var emitterSpring2:FlxEmitter;
	var fireSpring:FlxSprite;

	var fazbarsIP:FlxTypedGroup<FlxText>;

	var matpatBG:BGSprite;
	var matpatFloor:BGSprite;
	var matLogo:FlxSprite;
	var logoArrayAnim:Array<String>;
	var curMatAnim:Int = 1;
	...
	var santa:BGSprite;
	var tableKids:BGSprite;
	var blackScrAfton:FlxSprite;
	var midnightBlack:FlxSprite;
	var foregroundKids:BGSprite;
	var foregroundTable:BGSprite;
	var fred:BGSprite;
	var kids:BGSprite;
	var bloodLossLmao:BGSprite;
	var sbStage:FlxSprite;
	var sbError:FlxSprite;
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			detailsText = "Story Mode: Vs Afton";
		}
		else
		{
		...
		switch (curStage)
		{
			...
			case 'consequences':
				var bg:BGSprite = new BGSprite('backgrounds/' + curStage + '/pond', -880, -450, 1, 1);
				bg.setGraphicSize(Std.int(bg.width * 5.2));
				bg.antialiasing = false;
				bg.updateHitbox();
				add(bg);

			case 'aftonParty':
				var bg:BGSprite = new BGSprite('backgrounds/' + curStage + '/wall', -850, -350, 1, 1);
				bg.setGraphicSize(Std.int(bg.width * 2.8));
				bg.antialiasing = false;
				bg.updateHitbox();
				add(bg);

				var stageFront:BGSprite = new BGSprite('backgrounds/' + curStage + '/floor', -850, -350, 1, 1);
				stageFront.setGraphicSize(Std.int(stageFront.width * 2.8));
				stageFront.antialiasing = false;
				stageFront.updateHitbox();
				add(stageFront);

				tableKids = new BGSprite('backgrounds/' + curStage + '/children', -630, -350, 1, 1, ['children idle'], true);
				tableKids.setGraphicSize(Std.int(tableKids.width * 2.6));
				tableKids.antialiasing = false;
				tableKids.updateHitbox();
				add(tableKids);

				foregroundKids = new BGSprite('backgrounds/' + curStage + '/front', -760, -550, 1, 1, ['front kids'], true);
				foregroundKids.setGraphicSize(Std.int(foregroundKids.width * 2.8));
				foregroundKids.antialiasing = false;
				foregroundKids.updateHitbox();
			
			case 'aftonStage':
				var car:BGSprite = new BGSprite('backgrounds/' + curStage + '/outside', -620, -350, 1, 1);
				car.setGraphicSize(Std.int(car.width * 2.6));
				car.antialiasing = false;
				car.updateHitbox();
				add(car);
				
				var stageFront:BGSprite = new BGSprite('backgrounds/' + curStage + '/floor', -630, -350, 1, 1);
				stageFront.setGraphicSize(Std.int(stageFront.width * 2.6));
				stageFront.antialiasing = false;
				stageFront.updateHitbox();
				add(stageFront);

				kids = new BGSprite('backgrounds/' + curStage + '/kids_two', -632, -355, 1, 1, ['kids two kids two'], true);
				kids.setGraphicSize(Std.int(kids.width * 2.6));
				kids.antialiasing = false;
				kids.updateHitbox();
				add(kids);

				fred = new BGSprite('backgrounds/' + curStage + '/animatronics', -630, -355, 1, 1, ['animatronics animatronics'], true);
				fred.setGraphicSize(Std.int(fred.width * 2.6));
				fred.antialiasing = false;
				fred.updateHitbox();
				add(fred);
				...
				blackScrAfton.screenCenter(X);
				blackScrAfton.screenCenter(Y);
				add(blackScrAfton);

			case 'aftonBack':
				var bg:BGSprite = new BGSprite('backgrounds/' + curStage + '/wall', -630, -350, 1, 1);
				...
				var table:BGSprite = new BGSprite('backgrounds/' + curStage + '/table', -630, -350, 1, 1);
				table.setGraphicSize(Std.int(table.width * 2.6));
				table.antialiasing = false;
				table.updateHitbox();
				add(table);
	
				foregroundTable = new BGSprite('backgrounds/' + curStage + '/table_front', -630, -350, 1, 1);
				foregroundTable.setGraphicSize(Std.int(foregroundTable.width * 2.6));
				foregroundTable.antialiasing = false;
				foregroundTable.updateHitbox();
				add(foregroundTable);
	
				blackScrAfton = new FlxSprite().makeGraphic(FlxG.width * 3, FlxG.height + 700, FlxColor.BLACK);
				blackScrAfton.alpha = 0.5;
				blackScrAfton.screenCenter();

				midnightBlack = new FlxSprite().makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
				midnightBlack.alpha = 0.2;
				midnightBlack.screenCenter(X);
				midnightBlack.screenCenter(Y);

			case 'aftonKill':
				var bg:BGSprite = new BGSprite('backgrounds/' + curStage + '/wall', -800, -400, 1, 1);
				bg.setGraphicSize(Std.int(bg.width * 3));
				bg.antialiasing = false;
				bg.updateHitbox();
				add(bg);
		
				var stageFront:BGSprite = new BGSprite('backgrounds/' + curStage + '/floor', -800, -400, 1, 1);
				stageFront.setGraphicSize(Std.int(stageFront.width * 3));
				stageFront.antialiasing = false;
				stageFront.updateHitbox();
				add(stageFront);
		
				var table:BGSprite = new BGSprite('backgrounds/' + curStage + '/table', -680, -300, 1, 1);
				table.setGraphicSize(Std.int(table.width * 2.6));
				table.antialiasing = false;
				table.updateHitbox();
				add(table);
		
				foregroundTable = new BGSprite('backgrounds/' + curStage + '/table_front', -330, -400, 1, 1);
				foregroundTable.setGraphicSize(Std.int(foregroundTable.width * 2.6));
				foregroundTable.antialiasing = false;
				foregroundTable.updateHitbox();

				bloodLossLmao = new BGSprite('backgrounds/' + curStage + '/blood', -520, 100, 1, 1, ['blood blood'], true);
				bloodLossLmao.setGraphicSize(Std.int(bloodLossLmao.width * 4));
				bloodLossLmao.antialiasing = false;
				bloodLossLmao.alpha = 0;
				bloodLossLmao.updateHitbox();
				add(bloodLossLmao);
		
				blackScrAfton = new FlxSprite().makeGraphic(FlxG.width * 3, FlxG.height + 700, FlxColor.BLACK);
				blackScrAfton.alpha = 0.65;
				blackScrAfton.screenCenter();

			case 'sb':
				sbStage = new FlxSprite(-1030, -800);

				sbStage.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/shadow_bonnie');
				...
				sbError.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/error');
			...
			case 'matpat':
				var stageFront:BGSprite = new BGSprite('backgrounds/' + curStage + '/MatPat_Carpet', -300, -200, 1, 1);
				add(stageFront);

				var bg:BGSprite = new BGSprite('backgrounds/' + curStage + '/MatPat_Walls', -300, -200, 1, 1);
				add(bg);

				var couch:BGSprite = new BGSprite('backgrounds/' + curStage + '/MatPat_Couch', -300, -200, 1, 1);
				add(couch);

				var table:BGSprite = new BGSprite('backgrounds/' + curStage + '/MatPat_Table', -400, -200, 1, 1);
				add(table);

				matpatBG = new BGSprite('backgrounds/' + curStage + '/Background', -500, -200, 0, 0);
				matpatBG.alpha = 0;
				...
				matLogo.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/gt logo');
				...
				matpatFloor = new BGSprite('backgrounds/' + curStage + '/Floor', -800, -350, 1, 1);
				matpatFloor.alpha = 0;
			...
			case 'fright':
				var bg:BGSprite = new BGSprite('backgrounds/' + curStage + '/iminyourwalls', -50, -50, 1, 1);
				bg.setGraphicSize(Std.int(bg.width * 1.6));
				bg.antialiasing = true;
				add(bg);

				var floor:BGSprite = new BGSprite('backgrounds/' + curStage + '/floor', -50, -50, 1, 1);
				floor.setGraphicSize(Std.int(bg.width * 1.6));
				floor.antialiasing = true;
				add(floor);

				gradientSalvage = new BGSprite('backgrounds/' + curStage + '/gradient', -50, -50, 1, 1);
				gradientSalvage.setGraphicSize(Std.int(gradientSalvage.width * 1.6));
				gradientSalvage.antialiasing = true;

				puppet = new FlxSprite(0, -400);
				puppet.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/phantom_poopet');
				...
				chica.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/phantom_chica');
				...
				bb.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/phantom_bb');
				...
				salvageBlack = new FlxSprite().makeGraphic(FlxG.width * 3, FlxG.height + 700, FlxColor.BLACK);
				salvageBlack.alpha = 1;
				salvageBlack.screenCenter();
				salvageBlack.cameras = [camHUD];

				glitchSixAM = new FlxSprite(0, 0);
				glitchSixAM.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/nightBeatGlitch');
				glitchSixAM.animation.addByPrefix('glitch', "glitch", 12);
				glitchSixAM.alpha = 0;
				glitchSixAM.antialiasing = false;
				glitchSixAM.screenCenter(X);
				...
				sixAM.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/6am');
				sixAM.animation.addByPrefix('static', "6am static", 24);
				sixAM.animation.addByPrefix('chime', "6am chime", 24);
				sixAM.alpha = 0;
				sixAM.antialiasing = true;
				sixAM.screenCenter();
				sixAM.cameras = [camHUD];

				nightNewGlitch = new FlxSprite(0, 0);
				nightNewGlitch.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/newNightGlitch');
				...
				nightNewSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('backgrounds/fright/5thNight'));
			...
			case 'frightAAABURN':
				var bg:BGSprite = new BGSprite('backgrounds/' + curStage + '/Wall', -100, -30, 1, 1);
				bg.setGraphicSize(Std.int(bg.width * 1.6));
				bg.antialiasing = true;
				add(bg);

				var floor:BGSprite = new BGSprite('backgrounds/' + curStage + '/floor', -320, -250, 1, 1);
				floor.setGraphicSize(Std.int(floor.width * 1.6));
				floor.updateHitbox();
				floor.antialiasing = true;
				add(floor);

				var banner:BGSprite = new BGSprite('backgrounds/' + curStage + '/Banner', -100, -50, 1, 1);
				...
				smokeNight = new BGSprite('backgrounds/' + curStage + '/smokeyBacon', -100, -50, 1, 1);
				smokeNight.setGraphicSize(Std.int(smokeNight.width * 2));
				smokeNight.antialiasing = true;

				lightNight = new BGSprite('backgrounds/' + curStage + '/oraligh', -100, -50, 1, 1);
				...
				emitterSpring.loadParticles(Paths.image('backgrounds/frightAAABURN/ember1'), 500, 16, true);
				...
				emitterSpring2.loadParticles(Paths.image('backgrounds/frightAAABURN/ember1'), 500, 16, true);
				...
				springStatic.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/static');
			...
			case 'fazbars': //god help
				var bg:BGSprite = new BGSprite('backgrounds/fazbars/fazbars', -600, -450, 1, 1);
				bg.setGraphicSize(Std.int(bg.width * 3));
				bg.updateHitbox();
				add(bg);
		...
		if(curStage == 'consequences')
			add(boyfriendGroup);
		add(dadGroup);
		if (curStage != 'consequences')
			add(boyfriendGroup);
		if (curStage == 'aftonParty')
			add(foregroundKids);
		if (curStage == 'aftonBack'){
			add(blackScrAfton);	
		}
		if (curStage == 'aftonKill'){
			add(blackScrAfton);	
		}
		if (curStage == 'aftonBack'){
			add(midnightBlack);	
		...
		blackStartBG = new FlxSprite().makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
		blackStartBG.screenCenter(X);
		blackStartBG.screenCenter(Y);
		if (isPixelStage)
		{
			add(blackStartBG);
		...
		if (isPixelStage && curStage != 'consequences') {
			if(curStage != 'sb'){
				arcadeoverlay = new FlxSprite(0, 0).loadGraphic(Paths.image('backgrounds/ac'));
				arcadeoverlay.antialiasing = false;
				arcadeoverlay.setGraphicSize(Std.int(arcadeoverlay.width * 1));
				arcadeoverlay.updateHitbox();
				arcadeoverlay.screenCenter(X);
				arcadeoverlay.screenCenter(Y);
				arcadeoverlay.y -= 30;
				add(arcadeoverlay);
				arcadeoverlay.cameras = [camHUD];
			}
		}
		...
		if(isPixelStage){
			healthBarBG = new AttachedSprite('pixelUI/healthBar-pixel');
		}
		else{
		...
		if(isPixelStage){
			scoreTxt = new FlxText(0, healthBarBG.y + 36, FlxG.width, "", 20);
			scoreTxt.setFormat(Paths.font("retro_computer_personal_use.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			scoreTxt.scrollFactor.set();
			scoreTxt.borderSize = 1.25;
			scoreTxt.visible = !ClientPrefs.hideHud;
			add(scoreTxt);

			missTxt = new FlxText(scoreTxt.x - 400, healthBarBG.y + 36, FlxG.width, "", 40);
			missTxt.setFormat(Paths.font("retro_computer_personal_use.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			...
			accTxt = new FlxText(scoreTxt.x + 400, healthBarBG.y + 36, FlxG.width, "", 40);
			accTxt.setFormat(Paths.font("retro_computer_personal_use.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			accTxt.scrollFactor.set();
			accTxt.borderSize = 1.25;
			accTxt.visible = !ClientPrefs.hideHud;
			add(accTxt);
			accTxt.cameras = [camHUD];
		}

		botplayTxt = new FlxText(400, strumLine.y + 30, FlxG.width - 800, "BOTPLAY", 32);
		...
		if (isPixelStage) {
			scanline = new FlxSprite(0, 0).loadGraphic(Paths.image('backgrounds/scanline'));
			scanline.antialiasing = false;
			scanline.setGraphicSize(Std.int(scanline.width * 2));
			if (curStage != 'consequences')
				scanline.alpha = 0.5;
			scanline.updateHitbox();
			scanline.screenCenter(X);
			scanline.screenCenter(Y);
			add(scanline);
			scanline.cameras = [camHUD];
		}

		if (curStage == 'fright') {
			nightStart = false;

			puppet.x = strumLine.x + 630;
			add(puppet);
			add(chica);
			add(bb);
			add(salvageBlack);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case "you-can't":
					inCutscene = true;
					FlxG.sound.play(Paths.sound('aftonKill'));
					new FlxTimer().start(3, function(tmr:FlxTimer)
					{
						var yc:BGSprite = new BGSprite('backgrounds/aftonKill/you-cant', -800, -400, 1, 1);
						//yc.setGraphicSize(Std.int(yc.width * 3));
						yc.antialiasing = false;
						yc.updateHitbox();
						add(yc);
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(curStage == 'aftonParty') {
					foregroundKids.dance(true);
					tableKids.dance(true);
				}
				if(curStage == 'aftonStage') {
					kids.dance(true);
					fred.dance(true);
				}
				...
				if(curStage != 'fazbars'){
					...
					switch (swagCounter)
					{
						case 0:
							FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix), 0.6);
						case 1:
							var ready:FlxSprite = new FlxSprite().loadGraphic(Paths.image('backgrounds/fazbars/' + introAlts[0]));
							ready.scrollFactor.set();
							ready.updateHitbox();
							...
							add(ready);
							countDownSprites.push(ready);
							FlxTween.tween(ready, {y: ready.y += 100, alpha: 1}, Conductor.crochet / 1000, {
								ease: FlxEase.cubeInOut,
								onComplete: function(twn:FlxTween)
								{
									countDownSprites.remove(ready);
							...
							FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix), 0.6);
						case 2:
							var set:FlxSprite = new FlxSprite().loadGraphic(Paths.image('backgrounds/fazbars/' + introAlts[1]));
							set.scrollFactor.set();
							...
							add(set);
							countDownSprites.push(set);
							FlxTween.tween(set, {y: set.y += 100, alpha: 1}, Conductor.crochet / 1000, {
								ease: FlxEase.cubeInOut,
								onComplete: function(twn:FlxTween)
								{
									countDownSprites.remove(set);
							...
							FlxG.sound.play(Paths.sound('intro3' + introSoundsSuffix), 0.6);
						case 3:
							var go:FlxSprite = new FlxSprite().loadGraphic(Paths.image('backgrounds/fazbars/' + introAlts[2]));
							go.scrollFactor.set();
							...
							add(go);
							countDownSprites.push(go);
							FlxTween.tween(go, {y: go.y += 100, alpha: 1}, Conductor.crochet / 1000, {
								ease: FlxEase.cubeInOut,
								onComplete: function(twn:FlxTween)
								{
									countDownSprites.remove(go);
				...
				swagCounter += 1;
				// generateSong('fresh');
				if(isPixelStage && swagCounter >= 5)
				{
					//remove(blackStartBG);
					blackStartBG.destroy();
					camHUD.visible = true;
				}
			}, 5);
		}
	...
	function startSong():Void
	{
		...
		switch(curSong){
			...
			case "Midnight": curPortrait = "midnight";
			case "You-Cant": curPortrait = "you_cant";
			case "Just-A-Theory": curPortrait = "matpat";
			case "Fourth-Wall": curPortrait = "scott";
			case "Umbra": curPortrait = "rxq";
			case "Salvage": curPortrait = "salvage";
	...
	override public function update(elapsed:Float)
	{
		...
		if(isPixelStage){
			scoreTxt.text = 'SCR: ' + songScore;
			missTxt.text = 'MISS: ' + songMisses;
			if(ratingString == '?'){
				accTxt.text = 'ACC: ?%';
			} else{
				accTxt.text = 'ACC: ' + Math.floor(ratingPercent * 100) + '%';
			}
		}
		...
		if(umbraZoom){
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, CoolUtil.boundTo(1 - (elapsed * 20), 0, 1));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, CoolUtil.boundTo(1 - (elapsed * 20), 0, 1));
		}
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						if(daNote.noteType == 'GF Sing') {
							...
							dad.holdTimer = 0;
							if(curStage == 'sb' && dad.curCharacter == 'sb'){
	...
	function doDeathCheck() {
		if (health <= 0 && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				openSubState(new GameOverSubstate(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y, camFollowPos.x, camFollowPos.y, camHUD, this));
				for (tween in modchartTweens) {
					tween.active = true;
				}
				for (timer in modchartTimers) {
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Shadow Bonnie Error':
				dad.alpha = 0;
				boyfriend.alpha = 0;
				camHUD.visible = false;
				sbError.alpha = 1;
			
			case 'Shadow Bonnie Not Error':
			...
			case 'Midnight Black':
				...
				switch(charType) {
					case 0:
						midnightBlack.alpha = 0.4;
						defaultCamZoom += 0.1;
					case 1:
						midnightBlack.alpha += 0.2;
				}
			case 'Midnight Black Finale':
				var charType:Int = Std.parseInt(value1);
				if(Math.isNaN(charType)) charType = 0;
		
				switch(charType) {
					case 0:
						midnightBlack.alpha = 1;
						camHUD.visible = false;
			...
			case 'blood lmao':
				...
				switch(charType) {
					case 0:
						bloodLossLmao.alpha = 0;
					case 1:
						bloodLossLmao.alpha = 1;
				}
			case 'Matpat-to-Pixel':
				...
				remove(dadGroup);
				dadGroup.remove(dad);
				dad = new Character(0, 200, 'matpat-pixel');
				dadGroup.add(dad);
				add(dadGroup);

				remove(boyfriendGroup);
				boyfriendGroup.remove(boyfriend);
				boyfriend = new Boyfriend(400, 380, 'bf-matpat');
				boyfriendGroup.add(boyfriend);
				add(boyfriendGroup);
		
			case 'Matpat-to-Pixel-First':
				curMatAnim = 1;
				...
				remove(dadGroup);
				dadGroup.remove(dad);
				dad = new Character(0, 800, 'matpat-pixel');
				dadGroup.add(dad);
				add(dadGroup);
	
				remove(boyfriendGroup);
				boyfriendGroup.remove(boyfriend);
				boyfriend = new Boyfriend(400, 980, 'bf-matpat');
				boyfriendGroup.add(boyfriend);
				add(boyfriendGroup);
			...
			case 'Matpat-Zoom-Out':
				camHUD.zoom += 2.4;
				matLogo.animation.play('shinestat');
				defaultCamZoom = 0.65;
				isCameraOnForcedPos = false;
				camHUD.visible = true;
				FlxTween.tween(boyfriend, { y: 480}, 2, {ease: FlxEase.quadInOut});
				FlxTween.tween(dad, { y: 300}, 2, {ease: FlxEase.quadInOut});
				FlxTween.tween(matpatFloor, { y: -850}, 2, {ease: FlxEase.quadInOut});

			case 'Matpat-to-Normal':
				...
				remove(dadGroup);
				dadGroup.remove(dad);
				dad = new Character(-210, -240, 'matpat');
				dadGroup.add(dad);
				add(dadGroup);
	
				remove(boyfriendGroup);
				boyfriendGroup.remove(boyfriend);
				boyfriend = new Boyfriend(0, 350, 'bf');
				boyfriendGroup.add(boyfriend);
				add(boyfriendGroup);

			case 'scary':
				CoolUtil.browserLoad('https://youtu.be/KTDQ09oRS8I');
			case 'ip grabber':
				fazbarsIP = new FlxTypedGroup<FlxText>();
				add(fazbarsIP);

				for (i in 0...4){
					var ip:FlxText = new FlxText(-2000, 0, FlxG.width, "", 20);
					ip.setFormat(Paths.font("pixel.otf"), 50, FlxColor.BLACK, CENTER);
					...
					dots.setFormat(Paths.font("pixel.otf"), 50, FlxColor.BLACK, CENTER);
					dots.text = ".";
					dots.screenCenter();
					dots.x = ip.x + 70;
					if(i != 3)
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			if (SONG.validScore)
			{
				...
				{
					case 'salvage':
						AftonMenuState.salvageBeaten = true;

						FlxG.save.data.salvageBeaten = AftonMenuState.salvageBeaten;
						FlxG.save.flush();

					case 'nightmare':
						AftonMenuState.nightmareBeaten = true;

						FlxG.save.data.nightmareBeaten = AftonMenuState.nightmareBeaten;
						FlxG.save.flush();
					case 'umbra':
						AftonMenuState.umbraBeaten = true;
					
						FlxG.save.data.umbraBeaten = AftonMenuState.umbraBeaten;
						FlxG.save.flush();
					case 'just-a-theory':
						AftonMenuState.matpatBeaten = true;
				
						FlxG.save.data.matpatBeaten = AftonMenuState.matpatBeaten;
						FlxG.save.flush();
					case 'fourth-wall':
						AftonMenuState.scottBeaten = true;
			
						FlxG.save.data.scottBeaten = AftonMenuState.scottBeaten;
						FlxG.save.flush();
					case 'fazbars':
						AftonMenuState.fazbarsBeaten = true;
				
						FlxG.save.data.fazbarsBeaten = AftonMenuState.fazbarsBeaten;
						FlxG.save.flush();
				}
				#end
			}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					// if ()
					if(!usedPractice) {
						StoryMenuState.weekCompleted.set('afton', true);

						if (SONG.validScore)
						{
							Highscore.saveWeekScore('afton', campaignScore, storyDifficulty);
						}

						FlxG.save.data.weekCompleted = StoryMenuState.weekCompleted;
						FlxG.save.flush();

						AftonMenuState.springtrapUnlocked = true;

						FlxG.save.data.springtrapUnlocked = AftonMenuState.springtrapUnlocked;
						FlxG.save.flush();
					}
					usedPractice = false;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{
			rating.setGraphicSize(Std.int(rating.width * 1.3));
		...
		for (i in seperatedScore)
		{
			...
			if (!PlayState.isPixelStage)
			{
				...
				numScore.x = coolText.x + (43 * daLoop) + 180;
				numScore.y += 180;
			...
			{
				numScore.x = coolText.x + (43 * daLoop) - 140;
				numScore.y = strumLine.y + 70;
				//numScore.setGraphicSize(Std.int(numScore.width * 0.5));
			...
			if (!PlayState.isPixelStage)
			{
				...
				FlxTween.tween(numScore, {alpha: 1}, 0.1, {
					onComplete: function(tween:FlxTween)
					{
						numScore.destroy();
					},
		...
		if(!isPixelStage)
		{
			FlxTween.tween(comboSpr, {alpha: 0}, 1, {
				onComplete: function(tween:FlxTween)
				{
					coolText.destroy();
					comboSpr.destroy();
			...
			FlxTween.tween(comboSpr, {alpha: 1}, 0.1, {
				onComplete: function(tween:FlxTween)
				{
					coolText.destroy();
					comboSpr.destroy();
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		songMisses++;
		if (curStage == 'fazbars')
			FlxG.sound.play(Paths.sound('miss_faz'));
		vocals.volume = 0;
		RecalculateRating();
	...
	function shadowBonerStageSwap(){
		if(canChangeBoner == true){
			...
			var sbAnimPlay:Int = FlxG.random.int(0, 6);
	...
	public function umbraPlay(){
		FlxG.mouse.visible = false;
		PlayState.storyPlaylist = ['Umbra'];
		PlayState.isStoryMode = false;
			
		PlayState.SONG = Song.loadFromJson('umbra', 'umbra');
		PlayState.storyWeek = 0;
		PlayState.campaignScore = 0;

		AftonMenuState.shadowBonnieUnlocked = true;

		FlxG.save.data.shadowBonnieUnlocked = AftonMenuState.shadowBonnieUnlocked;
		FlxG.save.flush();
